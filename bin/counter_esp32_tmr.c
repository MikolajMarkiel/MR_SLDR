...
static int counter_esp32_init(const struct device *dev) {
  ...
  esp_intr_alloc(cfg->irq_source, 0, (ISR_HANDLER)counter_esp32_isr,
                 (void *)dev, NULL);
...
static int counter_esp32_cancel_alarm(const struct device *dev,
                                      uint8_t chan_id) {
  ARG_UNUSED(chan_id);
  struct counter_esp32_data *data = dev->data;
...
#ifdef CONFIG_COUNTER_AUTORELOAD
#define TIMER_AUTORELOAD TIMER_AUTORELOAD_EN
#else
#define TIMER_AUTORELOAD TIMER_AUTORELOAD_DIS
#endif
...
#define ESP32_COUNTER_INIT(idx)                                                \
  ...
  static const struct counter_esp32_config counter_config_##idx = {            \
      .counter_info = {.max_top_value = UINT32_MAX,                            \
                       .freq =                                                 \
                           (APB_CLK_FREQ / ESP32_COUNTER_GET_CLK_DIV(idx)),    \
      ...
      .config =                                                                \
          {                                                                    \
              ...
              .auto_reload = TIMER_AUTORELOAD, /*modified*/                    \
  ...
  DEVICE_DT_INST_DEFINE(idx, counter_esp32_init, NULL, &counter_data_##idx,    \
                        &counter_config_##idx, PRE_KERNEL_1,                   \
